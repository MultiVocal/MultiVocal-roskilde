...
public:
  bool setup(std::string clientId) {
    // set client id
    this->clientId = clientId;
    if (clientId == "") {
      std::cout << "No clientId assigned in config" << endl;
      return false;
    }
  ...
  void saveQueueToFile() {
    ...
    if (queue.empty()) {
      ...
      ofxJSON json;
      std::queue<file> tempQueue =
          queue; // copy the original queue to the temporary queue
  ...
  std::string getLastUploadCode() { return lastUploadCode; }
...
private:
  bool upload(file file) {
...
#ifdef TARGET_OSX
    pythonVersion =
        "/usr/local/bin/python " + path.getAbsolutePath("scripts/upload.py");
#elif __arm__
    pythonVersion =
        "/usr/bin/python " + path.getAbsolutePath("scripts/upload.py");
#else
    pythonVersion =
        "/usr/bin/python " + path.getAbsolutePath("scripts/upload.py");
#endif
    ...
    if (uploadSuccess) {
      ofLog(OF_LOG_NOTICE) << "Successfully uploaded: " << file.transcriptionId
                           << endl;
    }
