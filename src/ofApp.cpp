...
#ifdef TARGET_OSX
  audioRecorder.setFfmpegLocation(
      ofFilePath::getAbsolutePath("ffmpeg/ffmpeg_mac"));
  outputChannels = 1;
#elif __arm__
  audioRecorder.setFfmpegLocation(
      ofFilePath::getAbsolutePath("ffmpeg/ffmpeg_arm"));
  inputChannels = 2;
  outputChannels = 0;
...
// Setup serial in
  ...
  // setup uploader
  bool uploadSetup = fileUploader.setup(clientId);
...
void ofApp::draw() {
  ...
  std::string transcription =
      transcriptions[currentTranscriptionIndex]["transcription_text"]
          .asString();
  int numLines;
  ...
  if (serial.isInitialized() || !bDebugMode) {
    ...
    // Transcription
    font.drawMultiLineColumn(transcription, size, margin, y,
                             ofGetWidth() - margin, numLines, false, 2, true,
                             &wordsWereCropped, true);

    // Legal notice
    ofSetColor(0);
    font.drawMultiLineColumn(legalNotice, 18, margin, ofGetHeight() - 30,
                             ofGetWidth() - margin, numLines, false, 2, true,
                             &wordsWereCropped, true);
  } else {
    std::string notWorkingString = "The multivocal voicebox is currently out "
                                   "of order :(\ncheck back later!";
    std::string linkString = "multivocal.org";
    font.drawMultiLineColumn(notWorkingString, size, margin, y,
                             ofGetWidth() - margin, numLines, false, 2, true,
                             &wordsWereCropped, true);

    font.drawMultiLineColumn(linkString, 18, margin, ofGetHeight() - 30,
                             ofGetWidth() - margin, numLines, false, 2, true,
                             &wordsWereCropped, true);
  }
...
void ofApp::startRecording() {
  std::string timeStampFormat =
      std::string("%d_%b_%Y_" + ofToString("%Hh_") + ofToString("%Mm_") +
                  ofToString("%Ss"));
  ...
  std::string fileName = ofToString(currentTranscriptionIndex) + "_" +
                         ofGetTimestampString(timeStampFormat) + fileExt;
  ...
  // Start recording
  audioRecorder.setup(path + fileName, 0, 0, 0, sampleRate,
                      inputChannels); // no video
  audioRecorder.start();

  // Log
  ofLog(OF_LOG_NOTICE) << "Recording started for index: " +
                              ofToString(currentTranscriptionIndex)
                       << endl;
}
  ...
  if (file.exists()) {
    if (file.getSize() > minRecordingSize) {
      ...
      fileUploader.addFile(
          path, transcriptions[currentTranscriptionIndex]["transcription_id"]
                    .asString());
